		...
		$menulink.each(function(){
			...
			on('shown.bs.popover', function () {
				if (!elHasSub && $listWidget.length == 0){
					...
					// 多列菜单
					if ($el.hasClass('multi-cols-menu')) {
						var $colMenus = $listWidget.find('.col-menu'),
							colWidth = 0,
							colWidthTotal = 0;
						$colMenus.each(function(idx, elem) {
							colWidth = $(elem).width() + parseInt($(elem).css('paddingLeft').replace('px', ''), 10) * 2 + 30;
							$(elem).css({'width': colWidth});
							colWidthTotal += colWidth;
						});
						var width = (colWidthTotal + 34) + 'px';
						$('#' + $popover.attr('id')).css({'width': width});
						$('#' + $popover.attr('id')).css({'max-width': width});
					}
				}
				...
				popoverHeight	= $popcontent.find('.nano-content').children().css('height','auto').height();
				...
				if( offsetTop > offsetBottom){
					...
					offsetTop -= 5;
					$popcontent.css({'top': '','bottom': elHeight+'px', 'height': $popcontent.height()}).children().addClass('nano').css({'width':'100%'}).nanoScroller({
						preventPageScrolling : true
					});

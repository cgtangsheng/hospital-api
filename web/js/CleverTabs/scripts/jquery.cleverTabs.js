...
function CleverTabs(self, options) {
    ...
    this.options = $.extend({
        ...
        //Tab用于显示的Panel模板
        tabPanelTemplate: '<div id="cleverTabPanelItem-#{id}" class="clever-tab-content"><iframe frameBorder="0" id="#{id}" onload="setFrameHeight(\'#{id}\')" name="main" width="100%" marginwidth="0" style="overflow-x:hidden;" scrolling="auto" src="#{src}"></iframe></div>',
        //Tab唯一id生成器
...
CleverTabs.prototype.setupContextMenu = function () {
    ...
    if (!self.options.contextMenu) {
        contextMenu = {
            element: $('<ul id="cleverTabsContextMenu" class="ui-widget-content-context">'
...
CleverTab.prototype.activate = function () {
    ...
    if (function() {return /msie/.test(navigator.userAgent.toLowerCase());}) {
...
CleverTab.prototype.setLock = function (lock, changeOrgLock) {
    ...
    if (!lock) {
        ...
        this.header.append($('<span class="ui-icon ui-icon-locked" style="position: absolute; cursor: pointer; padding: 0px; top: 1px; right: 1px"></span>'));
    }
}
//获取指定Tab头完全id的唯一id

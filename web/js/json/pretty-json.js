...
window.CharCollapsed = ""; // +
window.ImgExpanded = "Expanded.gif";
window.CharExpanded = ""; // -
window.QuoteKeys = true;
window.defaultTabSize = 2;
...
function PrettyJson(JSONStr, isCollapsible) {
  SetTab();
  window.IsCollapsible = isCollapsible !== undefined ? isCollapsible : true;

  var html = '';
  try {
    if (isNaN(JSONStr) === false) {
      return false;
    }
    if (JSONStr == '') {
      JSONStr = "\"\"";
...
function ProcessObject(obj, indent, addComma, isArray, isPropertyContent){
  ...
  if(IsArray(obj)){
    if(obj.length == 0){
      ...
      clpsHtml = window.IsCollapsible ? "<span onClick=\"ExpImgClicked(this)\">"+window.CharExpanded+"</span><span class='collapsible'>" : "";
...
function SetTab(tabSize){
  ...
  tabSize = tabSize !== undefined ? tabSize : window.defaultTabSize;
